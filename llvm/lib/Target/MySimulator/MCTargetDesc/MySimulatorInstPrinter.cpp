...
#include "MySimulatorGenAsmWriter.inc"

void MySimulatorInstPrinter::printRegName(raw_ostream &O, unsigned RegNo) const {
  O << getRegisterName(RegNo);
}

void MySimulatorInstPrinter::printInst(const MCInst *MI, uint64_t Address,
...
void MySimulatorInstPrinter::printOperand(const MCInst *MI, int OpNo, raw_ostream &O) {
...
void MySimulatorInstPrinter::printBranchOperand(const MCInst *MI, uint64_t Address,
...
const char *MySimulatorInstPrinter::getRegisterName(unsigned RegNo) {
  return getRegisterName(RegNo,
                         ArchRegNames ? MySimulator::NoRegAltName : MySimulator::ABIRegAltName);
}
